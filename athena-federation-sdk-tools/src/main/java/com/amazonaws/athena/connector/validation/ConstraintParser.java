...
/**
 * This class provides the ability to transform a simple constraint grammar into
 * an instance of {@link Constraints}.
 */
public class ConstraintParser {
  ...
  private enum LogicalOperator {
    EQ("=", 0) {
      ValueSet createValueSet(ArrowType type, Object operand) {
        ...
        return SortedRangeSet.of(false,
                                 Range.equal(BLOCK_ALLOCATOR, type, operand));
      }
    },
    NEQ("!=", 1) {
      ValueSet createValueSet(ArrowType type, Object operand) {
    ...
    GT(">", 0) {
      ValueSet createValueSet(ArrowType type, Object operand) {
        return SortedRangeSet.of(
            false, Range.greaterThan(BLOCK_ALLOCATOR, type, operand));
      }
    },
    GTE(">=", 1) {
      ValueSet createValueSet(ArrowType type, Object operand) {
        return SortedRangeSet.of(
            false, Range.greaterThanOrEqual(BLOCK_ALLOCATOR, type, operand));
      }
    },
    LT("<", 0) {
      ValueSet createValueSet(ArrowType type, Object operand) {
        return SortedRangeSet.of(
            false, Range.lessThan(BLOCK_ALLOCATOR, type, operand));
      }
    },
    LTE("<=", 1) {
      ValueSet createValueSet(ArrowType type, Object operand) {
        return SortedRangeSet.of(
            false, Range.lessThanOrEqual(BLOCK_ALLOCATOR, type, operand));
      }
    };
   ...
   * @param input A comma-separated constraint String in the form of
   *     {field_name}{operator}{value}.
   *              The operators must be one of those available in {@link
   * LogicalOperator}. Currently, we only support Boolean, Integer, Floating
   * Point, Decimal, and String operands for this validator's constraints.
   * @return a {@link Constraints} object populated from the input string,
   *     un-constrained if input is not present
   */
  public static Constraints parseConstraints(Schema schema,
                                             Optional<String> input) {
    if (!input.isPresent() || input.get().trim().isEmpty()) {
      return new Constraints(Collections.EMPTY_MAP, Collections.emptyList(),
                             Collections.emptyList(), DEFAULT_NO_LIMIT);
    }

    Map<String, ArrowType> fieldTypes = schema.getFields().stream().collect(
    ...
    constraintStrings.forEach(
        str -> parseAndAddConstraint(fieldTypes, constraints, str));
    return new Constraints(constraints, Collections.emptyList(),
                           Collections.emptyList(), DEFAULT_NO_LIMIT);
  }
  ...
  private static void parseAndAddConstraint(Map<String, ArrowType> fieldTypes,
    ...
    for (LogicalOperator operator : LogicalOperator.values()) {
      if (constraintString.contains(operator.getOperator()) &&
          operator.getRank() > bestMatchRank) {
    ...
    if (fieldName == null) {
      throw new IllegalArgumentException(String.format(
          "Constraint segment %s could not be parsed into a valid expression!"
              +
              " Please use the form <field><operator><value> for each constraint.",
          constraintString));
    }

    // If there is no reference value for this field, then we treat the right
    // operand as null.
    ...
    if (operands.length > 1) {
      checkArgument(
          operands.length == 2,
          String.format(
              "Constraint argument %s contains multiple occurrences of operator %s",
              constraintString, matchedOperator.getOperator()));
      correctlyTypedOperand =
          tryParseOperand(fieldTypes.get(fieldName), operands[1]);
    }

    constraints.put(fieldName,
                    matchedOperator.createValueSet(fieldTypes.get(fieldName),
                                                   correctlyTypedOperand));
  }

  /*
   * Currently, we only support Boolean, Integer, Floating Point, Decimal, and
   * String operands.
   */
