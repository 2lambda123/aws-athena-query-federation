...
/**
 * This class should be used to validate deployed Lambda functions that use
 * Athena's Federation SDK. It simulates the basic query patterns that Athena
 * will send to Lambda throughout its usage so that broader and sometimes more
 * subtle logical issues can be discovered before being used through Athena. <p>
 * To run this tool using Maven, execute the following command:
 * mvn exec:java
 * -Dexec.mainClass=com.amazonaws.athena.connector.validation.AthenaConnectorValidator
 * -Dexec.args="[--lambda-func lambda_func [--record-func record_func]
 * [--catalog catalog] [--schema schema [--table table [--constraints
 * constraints]]] [--planning-only] [--help]]" <p> This tool can also be run
 * using the validate_connector.sh script in the tools directory under the
 * package root: tools/validate_connector.sh [args]
 */
public class ConnectorValidator {
  private static final Logger log =
      LoggerFactory.getLogger(ConnectorValidator.class);
   ...
   * [--schema schema [--table table [--constraints constraints]]]
   * [--planning-only] [--help] <p> Run with the -h or --help options to see
   * full argument descriptions, or see {@link TestConfig} below.
   */
  public static void main(String[] args) {
    try {
      ...
      final TableName table =
          testConfig.getTableId().isPresent()
      ...
      GetTableLayoutResponse tableLayout =
          getTableLayout(testConfig, table, schema, partitionColumns);

      GetSplitsResponse splitsResponse = getSplits(
          testConfig, table, schema, tableLayout, partitionColumns, null);

      if (!testConfig.isPlanningOnly()) {
        readRecords(testConfig, table, schema, splitsResponse.getSplits());
      } else {
        log.info(
            "Skipping record reading because the arguments indicated that only the planning should be validated.");
      }
      ...
      log.info("GetSplits included a continuation token: " +
               splitsResponse.getContinuationToken());
      log.info("Testing next batch of splits.");

      splitsResponse =
          getSplits(testConfig, table, schema, tableLayout, partitionColumns,
                    splitsResponse.getContinuationToken());
  ...
  private static Collection<String> showDatabases(TestConfig testConfig) {
    ListSchemasResponse schemasResponse = LambdaMetadataProvider.listSchemas(
        testConfig.getCatalogId(), testConfig.getMetadataFunction(),
    ...
    List<String> notLower = schemas.stream()
                                .filter(s -> !s.equals(s.toLowerCase()))
                                .collect(Collectors.toList());
    checkState(
        notLower.isEmpty(),
        "All returned schemas must be lowercase! Found these non-lowercase schemas: " +
            notLower);
    return schemas;
  }

  private static Collection<TableName> showTables(TestConfig testConfig,
                                                  String db) {
    ListTablesResponse tablesResponse = LambdaMetadataProvider.listTables(
        testConfig.getCatalogId(), db, testConfig.getMetadataFunction(),
        testConfig.getIdentity());
    ...
    if (!testConfig.isAllowEmptyTables()) {
      ...
      List<String> notLower =
          tables.stream()
              .filter(
                  t
                  -> !t.equals(new TableName(t.getSchemaName().toLowerCase(),
      ...
      checkState(
          notLower.isEmpty(),
          "All returned tables must be lowercase! Found these non-lowercase tables: " +
              notLower);
    } else {
      log.info("showTables is allowing empty table list, actual size is " +
               tables.size());
    }
    return tables;
  }

  private static GetTableResponse describeTable(TestConfig testConfig,
                                                TableName table) {
    GetTableResponse tableResponse = LambdaMetadataProvider.getTable(
        testConfig.getCatalogId(), table, testConfig.getMetadataFunction(),
        testConfig.getIdentity());
    TableName returnedTableName = tableResponse.getTableName();
    checkState(table.equals(returnedTableName),
               "Returned table name did not match the requested table name!"
                   + " Expected " + toQualifiedTableName(table) +
                   " but found " + toQualifiedTableName(returnedTableName));
    List<String> notLower = tableResponse.getSchema()
                                .getFields()
                                .stream()
                                .map(Field::getName)
    ...
    checkState(
        notLower.isEmpty(),
        "All returned columns must be lowercase! Found these non-lowercase columns: " +
            notLower);
    checkState(
        tableResponse.getSchema()
            .getFields()
            .stream()
            .map(Field::getName)
            .anyMatch(f -> !tableResponse.getPartitionColumns().contains(f)),
        "Table must have at least one non-partition column!");
    Set<String> fields = tableResponse.getSchema()
                             .getFields()
                             .stream()
                             .map(Field::getName)
                             .collect(Collectors.toSet());
    Sets.SetView<String> difference =
        Sets.difference(tableResponse.getPartitionColumns(), fields);
    checkState(
        difference.isEmpty(),
        "Table column list must include all partition columns! "
            +
  ...
  private static GetTableLayoutResponse
  getTableLayout(TestConfig testConfig, TableName table, Schema schema,
                 Set<String> partitionColumns) {
    Constraints constraints =
        parseConstraints(schema, testConfig.getConstraints());
    GetTableLayoutResponse tableLayout = LambdaMetadataProvider.getTableLayout(
        testConfig.getCatalogId(), table, constraints, schema, partitionColumns,
        testConfig.getMetadataFunction(), testConfig.getIdentity());
    log.info("Found " + tableLayout.getPartitions().getRowCount() +
             " partitions.");
    checkState(
        tableLayout.getPartitions().getRowCount() > 0,
  ...
  private static GetSplitsResponse getSplits(TestConfig testConfig,
    ...
    Constraints constraints =
        parseConstraints(schema, testConfig.getConstraints());
    GetSplitsResponse splitsResponse = LambdaMetadataProvider.getSplits(
        testConfig.getCatalogId(), table, constraints,
        tableLayout.getPartitions(), new ArrayList<>(partitionColumns),
        continuationToken, testConfig.getMetadataFunction(),
        testConfig.getIdentity());
    log.info("Found " + splitsResponse.getSplits().size() +
             " splits in batch.");
    if (continuationToken == null) {
      checkState(
          !splitsResponse.getSplits().isEmpty(),
          "Table " + toQualifiedTableName(table) +
  ...
  private static ReadRecordsResponse readRecords(TestConfig testConfig,
    ...
    log.info("Executing randomly selected split with properties: {}",
             split.getProperties());
    ReadRecordsResponse records = LambdaRecordProvider.readRecords(
        testConfig.getCatalogId(), table, constraints, schema, split,
        testConfig.getRecordFunction(), testConfig.getIdentity());
    log.info("Received " + records.getRecordCount() + " records.");
    checkState(
        records.getRecordCount() > 0,
        "Table " + toQualifiedTableName(table) +
  ...
  private static class TestConfig {
    ...
    private static final String ALLOW_EMPTY_SHOW_TABLES =
        "allow-empty-show-tables";
    ...
    private TestConfig(String metadataFunction, String recordFunction,
                       String catalogId, Optional<String> schemaId,
                       Optional<String> tableId, Optional<String> constraints,
                       boolean planningOnly, boolean allowEmptyTables) {
      ...
      this.identity = new FederatedIdentity(
          "VALIDATION_ARN", "VALIDATION_ACCOUNT", Collections.emptyMap(),
          Collections.emptyList());
    }

    public FederatedIdentity getIdentity() { return identity; }

    String getMetadataFunction() { return metadataFunction; }

    String getRecordFunction() { return recordFunction; }

    String getCatalogId() { return catalogId; }

    Optional<String> getSchemaId() { return schemaId; }

    Optional<String> getTableId() { return tableId; }

    Optional<String> getConstraints() { return constraints; }

    boolean isPlanningOnly() { return planningOnly; }

    boolean isAllowEmptyTables() { return allowEmptyTables; }
    ...
    static TestConfig fromArgs(String[] args) throws ParseException {
      ...
      options.addOption(
          "r", LAMBDA_RECORD_FUNCTION_ARG, true,
      ...
      if (parsedArgs.hasOption(HELP_ARG)) {
        new HelpFormatter().printHelp(
            150,
            "./validate_connector.sh --" + LAMBDA_METADATA_FUNCTION_ARG +
                " lambda_func [--" + LAMBDA_RECORD_FUNCTION_ARG +
                " record_func] [--" + CATALOG_ID_ARG + " catalog] [--" +
                SCHEMA_ID_ARG + " schema [--" + TABLE_ID_ARG + " table [--" +
                CONSTRAINTS_ARG + " constraints]]] [--" + PLANNING_ONLY_ARG +
                "]"
      ...
      checkArgument(
          parsedArgs.hasOption(LAMBDA_METADATA_FUNCTION_ARG),
          "Lambda function must be provided via the --lambda-func or -l args!");
      String metadataFunction =
          parsedArgs.getOptionValue(LAMBDA_METADATA_FUNCTION_ARG);
